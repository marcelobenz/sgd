                ...
                <div class="table-responsive">
                    <table class="table iso-table mb-0">
                        ...
                        @foreach($periodos as $p)
                            @php($puedeCerrar = $p->contextos_pendientes_count===0 && !is_null($p->cambio_climatico_relevante) && filled($p->fundamento_cambio_climatico))
                            <tr>
                                ...
                                <td class="text-right text-nowrap">
                                    @if($p->estado==='borrador')
                                        <form class="d-inline" method="POST" action="{{ route('planificacion.periodos.activar',$p) }}">@csrf @method('PATCH')<button class="btn btn-sm btn-outline-primary">Activar</button></form>
                                    @endif
                                    @if($p->estado!=='cerrado')
...
@foreach($periodos as $p)
    ...
    <div class="modal fade" id="cerrarPeriodo{{ $p->id }}" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog modal-lg" role="document"><form method="POST" action="{{ route('planificacion.periodos.cerrar',$p) }}" class="modal-content">@csrf @method('PATCH')
        ...
        <div class="modal-body">
            <div class="alert alert-warning"><strong>Al cerrar el período:</strong> no podrán agregarse ni modificarse elementos FODA, sus decisiones ni nuevos riesgos u oportunidades. Los datos y el informe permanecerán disponibles para consulta. El seguimiento posterior de acciones continúa habilitado.</div>
            <h6>Control previo</h6><div class="row text-center mb-3"><div class="col"><strong>{{ $p->contextos_pendientes_count }}</strong><br><small>FODA pendientes</small></div><div class="col"><strong>{{ $p->riesgos_abiertos_count }}</strong><br><small>Riesgos abiertos</small></div><div class="col"><strong>{{ $p->acciones_abiertas_count }}</strong><br><small>Acciones abiertas</small></div><div class="col"><strong>{{ $p->verificaciones_pendientes_count }}</strong><br><small>Verificaciones pendientes</small></div></div>
            @if($p->riesgos_abiertos_count || $p->acciones_abiertas_count || $p->verificaciones_pendientes_count)<p class="text-muted"><strong>Importante:</strong> estos pendientes no impiden el cierre, pero deberán continuar bajo seguimiento.</p>@endif
    ...
    <div class="modal fade" id="reabrirPeriodo{{ $p->id }}" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog modal-lg" role="document"><form method="POST" action="{{ route('planificacion.periodos.reabrir',$p) }}" class="modal-content">@csrf @method('PATCH')
        ...
        <div class="modal-body">
            <div class="alert alert-info"><strong>Al reabrir el período:</strong> volverán a habilitarse la carga y evaluación del FODA y la creación de riesgos u oportunidades. El período quedará como <strong>borrador</strong>; no reemplazará automáticamente al período vigente. La reapertura, su motivo y el responsable quedarán registrados.</div>
