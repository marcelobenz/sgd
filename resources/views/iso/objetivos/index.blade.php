...
<div class="iso-shell">
    ...
    <div class="iso-toolbar">
        <form method="get" class="d-flex flex-wrap gap-2 align-items-center">
            ...
            <select name="cumplimiento" class="form-select" onchange="this.form.submit()">
                <option value="">Todo cumplimiento</option>
                @foreach(['pendiente'=>'Pendiente de medición','cumplido'=>'Cumplido','aceptable'=>'Aceptable','incumplido'=>'Incumplido'] as $valor=>$texto)<option value="{{ $valor }}" @selected(request('cumplimiento')===$valor)>{{ $texto }}</option>@endforeach
            </select>
            <select name="situacion" class="form-select" onchange="this.form.submit()">
                <option value="">Todas las situaciones</option>
                @foreach(['activo'=>'Activo','cerrado'=>'Cerrado','suspendido'=>'Suspendido'] as $valor=>$texto)<option value="{{ $valor }}" @selected(request('situacion')===$valor)>{{ $texto }}</option>@endforeach
            </select>
            ...
            @if(request()->hasAny(['cumplimiento','situacion','proceso']))<a href="{{ route('planificacion.objetivos.index',['periodo'=>$periodo?->id]) }}" class="btn btn-link">Limpiar</a>@endif
        </form>
    ...
    <div class="iso-panel overflow-hidden">
        <div class="table-responsive">
            <table class="table iso-table mb-0">
                <thead><tr><th>Código</th><th>Objetivo</th><th>Proceso</th><th>Indicador y meta</th><th>Resultado actual</th><th>Acciones</th><th>Cumplimiento / situación</th><th></th></tr></thead>
                <tbody>
                @forelse($objetivos as $objetivo)
                    @php
                        ...
                        $evaluacionCierre = $objetivo->evaluaciones->firstWhere('tipo', 'cierre_periodo');
                    @endphp
                    <tr>
                        ...
                        <td><span class="iso-status {{ $estadoClase }}">{{ $estadoTexto }}</span><small class="d-block text-muted mt-1">{{ ucfirst($objetivo->estado) }}</small>@if($evaluacionCierre)<small class="d-block text-success mt-1"><i class="fa-solid fa-circle-check mr-1"></i>Cierre del período evaluado · {{ $evaluacionCierre->fecha_evaluacion->format('d/m/Y') }}</small>@else<small class="d-block text-warning mt-1"><i class="fa-regular fa-clock mr-1"></i>Pendiente de evaluación de cierre</small>@endif</td>
