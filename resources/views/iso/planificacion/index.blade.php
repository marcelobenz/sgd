...
<div class="iso-shell">
    ...
    <div class="iso-card-grid">
        ...
        <a class="iso-menu-card" href="{{ route('planificacion.objetivos.index') }}"><span class="iso-icon"><i class="fa-solid fa-bullseye"></i></span><div><h3>Objetivos de calidad</h3><p>Definí metas medibles, registrá resultados y evaluá su cumplimiento.</p>@if($stats['objetivos_activos'] || $stats['objetivo_acciones_vencidas'])<span class="iso-badge-count">{{ $stats['objetivos_activos'] }} activos · {{ $stats['objetivo_acciones_vencidas'] }} acciones vencidas</span>@endif</div></a>
        ...
        <a class="iso-menu-card" href="{{ route('planificacion.proveedores.index') }}"><span class="iso-icon"><i class="fa-solid fa-truck-field"></i></span><div><h3>Proveedores</h3><p>Seleccioná, evaluá y reevaluá proveedores externos, con acciones, evidencias y riesgos vinculados.</p>@if($stats['proveedores_vencidos'] || $stats['proveedor_acciones_vencidas'])<span class="iso-badge-count">{{ $stats['proveedores_vencidos'] }} evaluaciones · {{ $stats['proveedor_acciones_vencidas'] }} acciones vencidas</span>@endif</div></a>
