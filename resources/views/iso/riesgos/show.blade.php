...
<div class="iso-shell">
    ...
    <div class="row">
        <div class="col-lg-7 mb-4"><div class="iso-panel"><div class="iso-panel-header"><h2 class="iso-section-title mb-0">Acciones y evidencias</h2><span class="iso-status">{{ $riesgo->acciones->whereIn('estado',['completada','cancelada'])->count() }}/{{ $riesgo->acciones->count() }} resueltas</span></div><div class="iso-panel-body">
            @forelse($riesgo->acciones as $accion)
                <div class="border rounded p-3 mb-3"><div class="d-flex justify-content-between"><strong>{{ $accion->descripcion }}</strong><span class="iso-status {{ $accion->estado==='completada'?'ok':'' }}">{{ ucfirst(str_replace('_',' ',$accion->estado)) }}</span></div><small class="text-muted">{{ $accion->responsable?->name }} · Objetivo {{ $accion->fecha_objetivo?->format('d/m/Y') }}</small>
        ...
        <div class="col-lg-5 mb-4">
            <div class="iso-panel mb-4"><div class="iso-panel-header"><h2 class="iso-section-title mb-0">Historial de evaluaciones de eficacia</h2></div><div class="iso-panel-body">
                @forelse($riesgo->verificaciones->sortByDesc('fecha') as $verificacion)<div class="border-left pl-3 mb-3"><div class="d-flex justify-content-between"><strong>{{ $verificacion->fecha->format('d/m/Y') }} · {{ $verificacion->estado_resultante==='finalizado'?'Cierre del registro':'Evaluación de eficacia' }}</strong><span class="iso-status {{ $verificacion->eficacia==='si'?'ok':($verificacion->eficacia==='no'?'danger':'warn') }}">{{ ucfirst($verificacion->eficacia) }}</span></div><small class="text-muted">{{ $verificacion->verificador?->name }}</small><div>{{ $verificacion->conclusion }}</div>@if($verificacion->indice)<div>Valoración: {{ $verificacion->impacto }} × {{ $verificacion->probabilidad }} = {{ $verificacion->indice }} · {{ $verificacion->estado_resultante==='finalizado'?'Finalizado':'Continúa en tratamiento' }}</div>@endif @if($verificacion->justificacion_excepcion)<div><strong>Justificación:</strong> {{ $verificacion->justificacion_excepcion }}</div>@endif @if($verificacion->documento)<div><a href="{{ route('documentos.validaPermiso',['id'=>$verificacion->documento->id,'ruta'=>'documentos.show','permiso'=>'puedeLeer']) }}">Documento SGD: {{ $verificacion->documento->titulo }}</a></div>@endif @if($verificacion->enlace_externo)<div><a href="{{ $verificacion->enlace_externo }}" target="_blank" rel="noopener">Evidencia externa</a></div>@endif</div>@empty<p class="text-muted mb-0">Sin evaluaciones registradas.</p>@endforelse
            ...
            @if(auth()->user()->puedeGestionarPlanificacion() && $riesgo->estado!=='finalizado')
            <div class="iso-panel"><div class="iso-panel-header"><h2 class="iso-section-title mb-0">Evaluar eficacia</h2></div><form method="POST" action="{{ route('planificacion.riesgos.verificar',$riesgo) }}" class="iso-panel-body">@csrf @method('PATCH')
                ...
                <div class="form-group"><label>Resultado esperado / criterio de eficacia *</label><textarea name="criterio_eficacia" class="form-control" rows="3" required>{{ old('criterio_eficacia',$riesgo->criterio_eficacia) }}</textarea></div>
                <div class="form-row"><div class="form-group col-5"><label>Fecha de evaluación</label><input type="date" name="fecha" value="{{ old('fecha',now()->toDateString()) }}" class="form-control" required></div><div class="form-group col-7"><label>Resultado de eficacia</label><select name="eficacia" id="eficaciaEvaluacion" class="form-control"><option value="si" @selected(old('eficacia')==='si')>Sí — se alcanzó el resultado esperado</option><option value="parcial" @selected(old('eficacia','parcial')==='parcial')>Parcial — hubo mejora, pero falta completar</option><option value="no" @selected(old('eficacia')==='no')>No — no se alcanzó el resultado</option></select></div></div>
                <div class="form-group"><label>Conclusión y evidencia *</label><textarea name="conclusion" class="form-control" rows="4" required>{{ old('conclusion') }}</textarea></div>
                <div class="form-row"><div class="form-group col-6"><label>{{ $riesgo->tipo==='riesgo'?'Gravedad actual':'Beneficio actual' }}</label><select name="impacto_final" class="form-control">@foreach([1=>'1 — Bajo',2=>'2 — Medio',3=>'3 — Alto'] as $n=>$texto)<option value="{{ $n }}" @selected(old('impacto_final',$riesgo->impacto_final)==$n)>{{ $texto }}</option>@endforeach</select></div><div class="form-group col-6"><label>Probabilidad actual</label><select name="probabilidad_final" class="form-control">@foreach([1,2,3] as $n)<option value="{{ $n }}" @selected(old('probabilidad_final',$riesgo->probabilidad_final)==$n)>{{ $n }}</option>@endforeach</select></div></div>
                <div class="form-group"><label>Decisión posterior a la evaluación *</label><select name="decision" id="decisionEvaluacion" class="form-control" required><option value="continuar" @selected(old('decision','continuar')==='continuar')>Continuar tratamiento</option><option value="finalizar" @selected(old('decision')==='finalizar')>Finalizar riesgo u oportunidad</option></select><small class="form-text text-muted">Con eficacia parcial o no eficaz, lo habitual es continuar. El cierre excepcional requiere justificación.</small></div>
                <div class="form-group"><label>Próxima fecha de evaluación</label><input type="date" name="proxima_evaluacion" id="proximaEvaluacion" value="{{ old('proxima_evaluacion',$riesgo->fecha_verificacion_prevista?->toDateString()) }}" class="form-control"><small id="ayudaProximaEvaluacion" class="form-text text-muted">Obligatoria cuando se decide continuar el tratamiento.</small></div>
                <div class="form-group"><label>Justificación</label><textarea name="justificacion_excepcion" id="justificacionEvaluacion" class="form-control" rows="3" placeholder="Explicá la excepción o decisión adoptada cuando corresponda">{{ old('justificacion_excepcion') }}</textarea><small id="ayudaJustificacion" class="form-text text-muted">Se solicita únicamente cuando la combinación elegida requiere una explicación adicional.</small></div>
                <div class="form-row"><div class="form-group col-md-6"><select name="documento_id" class="form-control"><option value="">Sin documento</option>@foreach($documentos as $d)<option value="{{ $d->id }}">{{ $d->titulo }}</option>@endforeach</select></div><div class="form-group col-md-6"><input type="url" name="enlace_externo" class="form-control" placeholder="Enlace de evidencia"></div></div>
...
$(function () {
    ...
    function actualizarReglasEvaluacion() {
        ...
        const justificacionObligatoria = (continuar && eficacia === 'si') || (!continuar && eficacia !== 'si');
        ...
        if (continuar && eficacia === 'si') {
            ...
            $ayudaJustificacion.text('Puede ser requerida si se declara eficaz y la valoración no mejora respecto de la inicial.');
        }
    }

    $decision.add($eficacia).on('change', actualizarReglasEvaluacion);
