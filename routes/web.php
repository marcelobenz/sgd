...
Route::get('/go', function (Request $request) {
    ...
    // Sanitizar: sólo permitimos paths locales (evita open-redirect)
    $esSeguro = is_string($next)
    ...
    return auth()->check()
        ? redirect($next)                               // ya logueado → directo al doc
        : redirect()->route('login', ['next' => $next]); // no logueado → login con next
})->name('go');
